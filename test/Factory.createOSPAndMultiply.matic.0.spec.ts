...
describe("Factory.createOSPAndMultiply.matic.0", function () {
  ...
  describe("createOSPAndMultiply", function () {
    ...
    it("matic", async function () {
      ...
      const mortgageMatic1 = await ethers.provider.getBalance(
      ...
      const factoryMatic1 = await ethers.provider.getBalance(
        factoryContract.address
      );
      const factoryWmatic1 = await wmatic.balanceOf(factoryContract.address);
      ...
      const t1 = await factoryContract
        ...
        .createOSPAndMultiply(
          ...
          AllFindAmount,
          amountPayMax,
          { gasPrice: gasPrice, value: amountPayMax.add(osp1Params.buyNFTTokenAmountMax) }
        );
      ...
      const mortgageMatic2 = await ethers.provider.getBalance(
        mortgageContract.address
      );
      ...
      const factoryMatic2 = await ethers.provider.getBalance(
        factoryContract.address
      );
      const factoryWmatic2 = await wmatic.balanceOf(factoryContract.address);
      const factoryFind2 = await findContract.balanceOf(
        factoryContract.address
      );
      const factoryOsp2 = await osp1.balanceOf(factoryContract.address);

      expect(staticInfo.nftNeedPay).eq(0);
      expect(staticInfo.positionOspAmountDelta).eq(ospAmount5);
      expect(staticInfo.tokenPay).eq(wmatic.address);

      expect(staticInfo.amountNeedPay).eq(
        BigNumber.from("817068806908583810122")
      );

      expect(user1Find1).eq(user1Find2);
      expect(user1Wmatic1).eq(user1Wmatic2).eq(0);
      expect(user1Matic1.sub(user1Matic2).sub(t1w.gasUsed.mul(gasPrice))).eq(
        staticInfo.amountNeedPay
      );
      expect(staticInfo.nftNeedPay).eq(0);
      ...
      expect(await mortgageContract.ownerOf(staticInfo.tokenId)).eq(user1Wallet.address);
      ...
      expect(factoryWmatic2).eq(factoryWmatic1).eq(0);
      expect(factoryMatic2).eq(factoryMatic1).eq(0);
      ...
      expect(await findContract.balanceOf(factoryContract.address)).eq(928);
      expect(await wmatic.balanceOf(factoryContract.address)).eq(0);
      expect(await ethers.provider.getBalance(factoryContract.address)).eq(0);
