...
describe("Mortgage.mortgage.usdt", function () {
  ...
  describe("mortgage", function () {
    ...
    it("usdt", async function () {
      ...
      const mortgageUsdt1 = await usdtContract.balanceOf(
        mortgageContract.address
      );
      ...
      const user1Usdt2 = await usdtContract.balanceOf(user1Wallet.address);
      const earnUsdt2 = await usdtContract.balanceOf(earnContract.address);
      const mortgageUsdt2 = await usdtContract.balanceOf(
        mortgageContract.address
      );
      ...
      expect(mortgageInfo.amountOut).eq(outUsdt);
      expect(mortgageInfo.tokenOut).eq(usdtContract.address);
      ...
      const redeemUsdt = BigNumber.from("110313648276");

      expect(user1Find2).eq(user1Find3);
      expect(earnFind3).eq(earnFind2);
      expect(mortgageFind3).eq(mortgageFind2).eq(0);

      expect(user1Usdt2.sub(user1Usdt3)).eq(redeemUsdt);
      expect(earnUsdt3).eq(earnUsdt2).eq(0);
      expect(mortgageUsdt3).eq(mortgageUsdt3).eq(0);
      ...
      expect(redeemInfo.amountIn).eq(redeemUsdt);
      expect(redeemInfo.tokenIn).eq(usdtContract.address);
