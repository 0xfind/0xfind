...
describe("Mortgage.cash.usdt", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      const findToUsdtInPath =
        ...
        usdtContract.address.slice(2);
      console.log(1);
      const cashInfoUsdt = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(mortgageStaticInfo.tokenId, ospAmount5, findToUsdtInPath);
      const user1Usdt1 = await usdtContract.balanceOf(user1Wallet.address);
      console.log(2);
      await osp1
        .connect(deployWallet)
      ...
      await mortgageContract
        .connect(user1Wallet)
        .cash(mortgageStaticInfo.tokenId, ospAmount5, findToUsdtInPath);
      ...
      expect(user1Usdt4.sub(user1Usdt3)).eq(
        cashInfoUsdt.amountOut
      );
      ...
      expect(cashInfoUsdt.outFindAmount).eq(amountOut.sub(redeemFind));
      expect(cashInfoUsdt.tokenOut).eq(usdtContract.address);
      expect(cashInfoUsdt.amountOut).lt(cashInfoUsdt.outFindAmount);

      expect(await findContract.totalSupply()).eq(findOldTotalSupply);

      expect(BigNumber.from("110313878720")).eq(redeemInfoUsdt.amountIn)
      expect(BigNumber.from("228053479808")).eq(cashInfoUsdt.amountOut)
