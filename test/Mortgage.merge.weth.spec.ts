...
describe("Mortgage.merge.weth", function () {
  ...
  describe("merge", function () {
    ...
    it("xxx", async function () {
      ...
      const mergeInfo = await mortgageContract
        .connect(user1Wallet)
        .callStatic.merge(info1.tokenId, info2.tokenId, findToWethInPath);
      await mortgageContract
        .connect(user1Wallet)
        .merge(info1.tokenId, info2.tokenId, findToWethInPath);
      ...
      expect(user1Weth4.sub(user1Weth3)).eq(BigNumber.from("3228940551708328587"));
      expect(earnWeth4).eq(earnWeth3);
      expect(mortgageWeth4).eq(mortgageWeth3);
      ...
      expect(mergeInfo.amountOut).eq(
        BigNumber.from("3228940551708328587")
      );
