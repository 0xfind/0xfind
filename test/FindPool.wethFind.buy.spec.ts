...
import { UNISWAP_ROUTER, deployAllContractFindWeth } from "./share/utils";

import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";

describe("FindPool.wethFind.buy", function () {
  let wallets;
  ...
  before(async function () {
    let allInfo = await deployAllContractFindWeth();
  ...
  it("buy", async function () {
    ...
    console.log("0 is weth, 1 is find");
    expect(await findPool.fee())
      .eq(findinfo.fee)
      .eq(100);
    expect(await findPool.token0()).eq(wethContract.address);
    expect(await findPool.token1()).eq(findContract.address);
    ...
    expect(buy1info.weth).lt(BigNumber.from(10).pow(11).mul(10002));

    // buy 95% find
    const buy2info = await buyFind(
      BigNumber.from(10).pow(18).mul(950_0000_0000)
    );
    expect(buy2info.find).eq(BigNumber.from(10).pow(18).mul(950_0000_0000));

    expect(buy2info.weth).gt(
      BigNumber.from(10).pow(11).mul(950_0000_0000_0000)
    );
    expect(buy2info.weth).lt(
      BigNumber.from(10).pow(11).mul(950_1900_0000_0000)
    );
  });
});
