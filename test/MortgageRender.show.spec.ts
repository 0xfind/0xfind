...
describe("MortgageRender.show", function () {
  ...
  describe("show", function () {
    ...
    it("show", async function () {
      ...
      const show = await mortgageContract.tokenURI(mortgageInfo.tokenId);
      await saveNFTSVG("mortgage.int", show);

      await mortgageContract
        .connect(user1Wallet)
        .mortgageAdd(
          mortgageInfo.tokenId,
          BigNumber.from(10).pow(15).mul(500),
          findContract.address
        );

      const show2 = await mortgageContract.tokenURI(mortgageInfo.tokenId);
      await saveNFTSVG("mortgage.dec1", show2);

      await mortgageContract
        .connect(user1Wallet)
        .mortgageAdd(
          mortgageInfo.tokenId,
          BigNumber.from(10).pow(15).mul(510),
          findContract.address
        );

      const show3 = await mortgageContract.tokenURI(mortgageInfo.tokenId);
      await saveNFTSVG("mortgage.dec2", show3);

      await mortgageContract
        .connect(user1Wallet)
        .mortgageAdd(
          mortgageInfo.tokenId,
          BigNumber.from(10).pow(15).mul(1200),
          findContract.address
        );

      const show4 = await mortgageContract.tokenURI(mortgageInfo.tokenId);
      await saveNFTSVG("mortgage.dec3", show4);
