...
describe("Mortgage.cash.approve", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      const cashEarnFind1 = await findContract.balanceOf(earnContract.address);
      const cashUser1Find1 = await findContract.balanceOf(user1Wallet.address);
      const cashUser2Find1 = await findContract.balanceOf(user2Wallet.address);
      const cashMortgageFind1 = await findContract.balanceOf(mortgageContract.address);
      const cashEarnOsp1 = await osp1.balanceOf(earnContract.address);
      const cashUser1Osp1 = await osp1.balanceOf(user1Wallet.address);
      const cashUser2Osp1 = await osp1.balanceOf(user2Wallet.address);
      const cashMortgageOsp1 = await osp1.balanceOf(mortgageContract.address);
  
      const cashInfoSplit1 = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount1.mul(2), findContract.address);
      await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount1.mul(2), findContract.address);

      const cashEarnFind2 = await findContract.balanceOf(earnContract.address);
      const cashUser1Find2 = await findContract.balanceOf(user1Wallet.address);
      const cashUser2Find2 = await findContract.balanceOf(user2Wallet.address);
      const cashMortgageFind2 = await findContract.balanceOf(mortgageContract.address);
      const cashEarnOsp2 = await osp1.balanceOf(earnContract.address);
      const cashUser1Osp2 = await osp1.balanceOf(user1Wallet.address);
      const cashUser2Osp2 = await osp1.balanceOf(user2Wallet.address);
      const cashMortgageOsp2 = await osp1.balanceOf(mortgageContract.address);
  
      expect(cashEarnFind2).eq(cashEarnFind1);
      expect(cashUser1Find2.sub(cashUser1Find1)).eq(
        cashInfoSplit1.outFindAmount
      )
      expect(cashUser2Find2).eq(cashUser2Find1);
      expect(cashMortgageFind2).eq(cashMortgageFind1).eq(0);
      expect(cashEarnOsp2).eq(cashEarnOsp1).eq(0);
      expect(cashUser1Osp2).eq(cashUser1Osp1);
      expect(cashUser2Osp2).eq(cashUser2Osp1);
      expect(cashMortgageOsp1.sub(cashMortgageOsp2)).eq(
        ospAmount1.mul(2)
      );

      await mortgageContract.connect(user1Wallet).approve(user2Wallet.address, position.tokenId)
      ...
      const cashInfoSplit3 = await mortgageContract
        .connect(user2Wallet)
        .callStatic.cash(position.tokenId, ospAmount1, findContract.address);
      await mortgageContract
        .connect(user2Wallet)
        .cash(position.tokenId, ospAmount1, findContract.address);

      const cashEarnFind3 = await findContract.balanceOf(earnContract.address);
      const cashUser1Find3 = await findContract.balanceOf(user1Wallet.address);
      const cashUser2Find3 = await findContract.balanceOf(user2Wallet.address);
      const cashMortgageFind3 = await findContract.balanceOf(mortgageContract.address);
      const cashEarnOsp3 = await osp1.balanceOf(earnContract.address);
      const cashUser1Osp3 = await osp1.balanceOf(user1Wallet.address);
      const cashUser2Osp3 = await osp1.balanceOf(user2Wallet.address);
      const cashMortgageOsp3 = await osp1.balanceOf(mortgageContract.address);
  
      expect(cashEarnFind3).eq(cashEarnFind2);
      expect(cashInfoFind.tokenOut).eq(findContract.address);
      expect(cashUser1Find3).eq(cashUser1Find2);
      expect(cashUser2Find3.sub(cashUser2Find2)).eq(
        cashInfoSplit3.amountOut.add(cashInfoSplit2.amountOut)
      );
      expect(cashMortgageFind3).eq(cashMortgageFind2).eq(0);
      expect(cashEarnOsp3).eq(cashEarnOsp2).eq(0);
      expect(cashUser1Osp3).eq(cashUser1Osp2);
      expect(cashUser2Osp3).eq(cashUser2Osp2);
      expect(cashMortgageOsp2.sub(cashMortgageOsp3)).eq(
        ospAmount1.mul(3)
      );

      expect(
        cashUser2Find3.sub(cashUser2Find2).add(
          cashUser1Find2.sub(cashUser1Find1)
        )
      ).eq(cashInfoFind.amountOut.sub(1));
