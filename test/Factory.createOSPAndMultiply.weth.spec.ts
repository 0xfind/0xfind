...
describe("Factory.createOSPAndMultiply.weth", function () {
  ...
  describe("createOSPAndMultiply", function () {
    ...
    it("weth", async function () {
      ...
      await wethContract
        .connect(user1Wallet)
        .approve(factoryContract.address, amountPayMax.add(osp1Params.buyNFTTokenAmountMax));
      ...
      expect(staticInfo.amountNeedPay).eq(
        BigNumber.from("1609533102699111032")
      );

      expect(user1Find1).eq(user1Find2);
      expect(user1Weth1.sub(user1Weth2)).eq(staticInfo.amountNeedPay.add(staticInfo.nftNeedPay));
      ...
      expect(mortgageWeth2).eq(mortgageWeth1);
