...
describe("Mortgage.split.matic", function () {
  ...
  describe("split", function () {
    ...
    it("xxx", async function () {
      ...
      await mortgageContract
        .connect(user1Wallet)
        .mortgage(osp1.address, ospAmount5.mul(2), findContract.address);
      const user1Find2 = await findContract.balanceOf(user1Wallet.address);

      expect(user1Find2.sub(user1Find1)).eq(
        BigNumber.from("215313151921146970608892")
      );
      expect(await findContract.totalSupply()).eq(
        findOldTotalSupply.add(BigNumber.from("216395127558941679003911"))
      );

      const needFind = BigNumber.from("216395127558941679003911").sub(
        BigNumber.from("106574803158593914169768").mul(2)
      );
      const amountInMax = BigNumber.from(10).pow(18).mul(20_0000);

      const position3 = await mortgageContract.positions(info.tokenId);
      expect(position3.ospAmount).eq(ospAmount5.mul(2));
      ...
      const splitInfo = await mortgageContract
        .connect(user1Wallet)
        .callStatic.split(
          info.tokenId,
          ospAmount5,
      ...
      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, 0, amountInMax, maticToFindOutPath, {
            value: amountInMax,
          })
      ).revertedWith("E3");
      ...
      const mortgageMatic3 = await ethers.provider.getBalance(
        mortgageContract.address
      );
      ...
      const t1 = await mortgageContract
        .connect(user1Wallet)
        .split(info.tokenId, ospAmount5, amountInMax, maticToFindOutPath, {
          value: amountInMax, gasPrice: gasPrice
        });
      ...
      expect(user1Find3).eq(user1Find4);
      expect(mortgageFind4).eq(mortgageFind3).eq(0);

      expect(user1Wmatic4).eq(user1Wmatic3);
      expect(earnWmatic4).eq(earnWmatic3);
      expect(mortgageWmatic4).eq(mortgageWmatic3);

      expect(user1Matic3.sub(user1Matic4).sub(gas)).eq(splitInfo.amountIn);
      expect(earnMatic4).eq(earnMatic3).eq(0);
      expect(mortgageMatic4).eq(mortgageMatic3).eq(0);
      ...
      const position4 = await mortgageContract.positions(info.tokenId);
      const newPosition = await mortgageContract.positions(
        splitInfo.newTokenId
      );
      expect(position4.tokenId).eq(info.tokenId);
      expect(position4.ospAsset).eq(osp1.address);
      expect(position4.ospAmount).eq(ospAmount5);

      expect(newPosition.tokenId).eq(splitInfo.newTokenId);
      expect(newPosition.ospAsset).eq(osp1.address);
      expect(newPosition.ospAmount).eq(ospAmount5);

      // 2wei is the expected deviation within the mortgage pool
      expect(splitInfo.needFind).eq(needFind.sub(2));
      expect(splitInfo.amountIn).eq(BigNumber.from("1647716749925610619440"));
      expect(splitInfo.tokenIn).eq(wmatic.address);
