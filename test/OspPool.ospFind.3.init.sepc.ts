...
describe("OspPool.ospFind.init", function () {
  let wallets;
  ...
  it("projectId2OspToken token2OspInfo ospLpTokenIdList", async function () {
    ...
    expect(ospLpTokenIdList.length).eq(iospLpTokenIdList.length).eq(4);
  ...
  it("osp pool info", async function () {
    ...
    expect(await ospPool.fee())
      .eq(DEFAULT_OSP_POOL_CONFIG_3.fee)
      .eq(10000);
    expect(await ospPool.token0()).eq(osp1.address);
    ...
    expect(tmp1).lt(1000);
    expect(tmp2.add(tmp1)).eq(BigNumber.from(10).pow(18).mul(680_0000));
  });
  ...
  it("osp nft info", async function () {
    ...
    expect(cnftTokenId).eq(8);
    expect(onftTokenId).eq(9);
  ...
  it("osp lp token info", async function () {
    ...
    expect(ospLpTokenIdList.length).eq(4);
    ...
    const slot = await ospPool.slot0();
    console.log("sqrtPriceX96", slot.sqrtPriceX96);
    console.log("tick", slot.tick);
    ...
    for (let index = 0; index < ospLpTokenIdList.length; index++) {
      ...
      const tickLowerInfo = await ospPool.ticks(position.tickLower);
      console.log("liquidityNet", tickLowerInfo.liquidityNet);
      console.log("liquidityGross", tickLowerInfo.liquidityGross);
      
      console.log("tickUpper", position.tickUpper);
      const tickUpperInfo = await ospPool.ticks(position.tickUpper);
      console.log("liquidityNet", tickUpperInfo.liquidityNet);
      console.log("liquidityGross", tickUpperInfo.liquidityGross);

      expect(position.token0).eq(osp1.address);
      expect(position.token1).eq(findContract.address);
      expect(position.fee).eq(DEFAULT_OSP_POOL_CONFIG_3.fee).eq(config.fee).eq(10000);
    }
  });
