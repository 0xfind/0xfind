...
describe("Mortgage.cash.weth", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      const cashInfoWeth = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(mortgageStaticInfo.tokenId, ospAmount5, findToWethInPath);
      ...
      await mortgageContract
        .connect(user1Wallet)
        .cash(mortgageStaticInfo.tokenId, ospAmount5, findToWethInPath);
      ...
      expect(user1Weth4.sub(user1Weth3)).eq(
        cashInfoWeth.amountOut
      );
      ...
      expect(cashInfoWeth.outFindAmount).eq(amountOut.sub(redeemFind));
      expect(cashInfoWeth.tokenOut).eq(wethContract.address);
      expect(cashInfoWeth.amountOut).lt(cashInfoWeth.outFindAmount);

      expect(await findContract.totalSupply()).eq(findOldTotalSupply);
      expect(BigNumber.from("106584466338176602638")).eq(redeemInfoWeth.amountIn)
      expect(BigNumber.from("224818215686858681378")).eq(cashInfoWeth.amountOut)
    });
  });
});
