...
describe("Mortgage.cash.split.usdt", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      const cashInfoUsdt = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount5, findToUsdtInPath);
      ...
      const cashInfoSplit1 = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount1.mul(2), findToUsdtInPath);
      await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount1.mul(2), findToUsdtInPath);
      ...
      const cashInfoSplit3 = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount1, findToUsdtInPath);
      await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount1, findToUsdtInPath);
      ...
      expect(user1Usdt4.sub(user1Usdt3))
      .eq(cashInfoUsdt.amountOut.sub(1))
      .eq(
      ...
      expect(cashInfoSplit1.tokenOut).eq(usdtContract.address);
      expect(cashInfoSplit2.tokenOut).eq(usdtContract.address);
      expect(cashInfoSplit3.tokenOut).eq(usdtContract.address);

      expect(cashInfoUsdt.outFindAmount.sub(1)).eq(
        cashInfoSplit1.outFindAmount
          .add(cashInfoSplit2.outFindAmount)
          .add(cashInfoSplit3.outFindAmount)
      );

      console.log("find", cashInfoUsdt.outFindAmount);
      console.log("usdt", cashInfoUsdt.amountOut);
