...
describe("Mortgage.multiply.add.usdt", function () {
  ...
  describe("multiply", function () {
    ...
    it("xxx", async function () {
      ...
      await usdtContract
        .connect(deployWallet)
        .transfer(user2Wallet.address, BigNumber.from(10).pow(6).mul(20_0000).mul(4));

      // user2 multiply 2
      // user2 multiply 2
      // user2 multiply 1
      const AllFindAmount = BigNumber.from("107651316321812034514918");
      const amountPayMax = BigNumber.from(10).pow(6).mul(20_0000)
      const user2AllFindAmount1 = BigNumber.from("43060526528724813805966");
      const user2amountPayMax1 = BigNumber.from(10).pow(6).mul(20_0000)
      const user2AllFindAmount2 = BigNumber.from("43060526528724813805966");
      const user2amountPayMax2 = BigNumber.from(10).pow(6).mul(20_0000)
      const user2AllFindAmount3 = BigNumber.from("21530263264362406902986");
      const user2amountPayMax3 = BigNumber.from(10).pow(6).mul(20_0000)
      ...
      expect(user2Usdt1.sub(user2Usdt2))
      .eq(user2MultiplyInfo.amountNeedPay.add(2))
      .eq(
      ...
      expect(user2MultiplyInfoSplit1.tokenPay).eq(usdtContract.address);
      expect(user2MultiplyInfoSplit2.tokenPay).eq(usdtContract.address);
      expect(user2MultiplyInfoSplit3.tokenPay).eq(usdtContract.address);

      console.log("usdt", user2MultiplyInfo.amountNeedPay);
