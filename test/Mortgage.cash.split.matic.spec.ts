...
describe("Mortgage.cash.split.matic", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      const cashInfoMatic = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount5, findToMaticInPath);
      ...
      const cashInfoSplit1 = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount1.mul(2), findToMaticInPath);
      const t1 = await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount1.mul(2), findToMaticInPath, {gasPrice: gasPrice});
      ...
      const cashInfoSplit3 = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(position.tokenId, ospAmount1, findToMaticInPath);
      const t3 = await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount1, findToMaticInPath, {gasPrice: gasPrice});
      ...
      const mortgageMatic4 = await ethers.provider.getBalance(
        mortgageContract.address
      );
      ...
      expect(user1Matic4.sub(user1Matic3).add(gas))
      .eq(cashInfoMatic.amountOut.sub(1030))
      .eq(
      ...
      expect(cashInfoSplit1.tokenOut).eq(wmatic.address);
      expect(cashInfoSplit2.tokenOut).eq(wmatic.address);
      expect(cashInfoSplit3.tokenOut).eq(wmatic.address);

      expect(cashInfoMatic.outFindAmount.sub(1)).eq(
        cashInfoSplit1.outFindAmount
          .add(cashInfoSplit2.outFindAmount)
          .add(cashInfoSplit3.outFindAmount)
      );

      console.log("find", cashInfoMatic.outFindAmount);
      console.log("matic", cashInfoMatic.amountOut);
