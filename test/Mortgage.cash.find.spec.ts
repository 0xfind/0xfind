...
describe("Mortgage.cash.find", function () {
  ...
  describe("cash", function () {
    ...
    it("xxx", async function () {
      ...
      await expect(
        mortgageContract
          .connect(user1Wallet)
          .cash(position.tokenId, 0, findContract.address)
      ).revertedWith("E3");

      await expect(
        mortgageContract
          .connect(user1Wallet)
          .cash(position.tokenId, ospAmount5.add(1), findContract.address)
      ).revertedWith("E2");

      await mortgageContract
        .connect(user1Wallet)
        .cash(position.tokenId, ospAmount5, findContract.address);
      ...
      expect(user1Find4.sub(user1Find3)).eq(amountOut.sub(redeemFind));
      ...
      const position2 = await mortgageContract.positions(position.tokenId);
      expect(position2.ospAmount).eq(0);
      expect(position2.ospAsset).eq(ZERO_ADDRESS);

      expect(cashInfoFind.outFindAmount).eq(amountOut.sub(redeemFind));
      expect(cashInfoFind.tokenOut).eq(findContract.address);
      expect(cashInfoFind.amountOut).eq(amountOut.sub(redeemFind));
