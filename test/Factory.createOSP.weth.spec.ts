...
describe("Factory.createOSP.weth", function () {
  ...
  describe("createOSP", function () {
    ...
    it("weth", async function () {
      ...
      expect(staticInfo.nftNeedPay).eq(BigNumber.from("3000272011027972"));
      expect(staticInfo.tokenPay).eq(wethContract.address);

      expect(user1Find1).eq(user1Find2);
      expect(user1Weth1.sub(user1Weth2)).eq(staticInfo.nftNeedPay);
