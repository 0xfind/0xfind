...
describe("Mortgage.split.approve", function () {
  ...
  describe("split", function () {
    ...
    it("xxx", async function () {
      ...
      await mortgageContract.connect(user1Wallet).approve(user2Wallet.address, info.tokenId);
      ...
      const user2Osp4 = await osp1.balanceOf(user2Wallet.address);
      ...
      // 2wei is the expected deviation within the mortgage pool
      expect(user2Find3.sub(user2Find4)).eq(needFind.sub(2));
      expect(mortgageFind4).eq(mortgageFind3).eq(0);
      expect(user1Osp4).eq(user1Osp3).eq(0);
      expect(user2Osp4).eq(user2Osp3).eq(0);
