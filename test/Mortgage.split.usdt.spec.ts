...
describe("Mortgage.split.usdt", function () {
  ...
  describe("split", function () {
    ...
    before(async function () {
      ...
      await usdtContract
        .connect(deployWallet)
        .transfer(user1Wallet.address, BigNumber.from(10).pow(6).mul(20_0000));
    ...
    it("xxx", async function () {
      ...
      const amountInMax = BigNumber.from(10).pow(6).mul(20_0000);
      ...
      const user1Usdt3 = await usdtContract.balanceOf(user1Wallet.address);
      const earnUsdt3 = await usdtContract.balanceOf(earnContract.address);
      const mortgageUsdt3 = await usdtContract.balanceOf(
        mortgageContract.address
      );
      ...
      await usdtContract
        .connect(user1Wallet)
        .approve(mortgageContract.address, amountInMax);
      ...
      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, 0, amountInMax, otherToFindOutPath)
      ).revertedWith("E3");

      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, ospAmount5.mul(2), amountInMax, otherToFindOutPath)
      ).revertedWith("AE");

      await mortgageContract
        .connect(user1Wallet)
        .split(info.tokenId, ospAmount5, amountInMax, otherToFindOutPath);
      ...
      const mortgageFind4 = await findContract.balanceOf(
        mortgageContract.address
      );

      const user1Usdt4 = await usdtContract.balanceOf(user1Wallet.address);
      const earnUsdt4 = await usdtContract.balanceOf(earnContract.address);
      const mortgageUsdt4 = await usdtContract.balanceOf(
        mortgageContract.address
      );
      ...
      expect(user1Usdt3.sub(user1Usdt4)).eq(splitInfo.amountIn);
      expect(earnUsdt4).eq(earnUsdt3).eq(0);
      expect(mortgageUsdt4).eq(mortgageUsdt3).eq(0);
      ...
      expect(splitInfo.amountIn).eq(BigNumber.from("3359383813"));
      expect(splitInfo.tokenIn).eq(usdtContract.address);
