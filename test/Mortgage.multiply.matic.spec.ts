...
describe("Mortgage.multiply.matic", function () {
  ...
  describe("multiply", function () {
    ...
    before(async function () {
      ...
      await swapRouter.connect(deployWallet).refundETH();
    ...
    it("xxx", async function () {
      ...
      await expect(
        mortgageContract
          ...
          .multiply(
            ...
            info.amountNeedPay,
            otherToFindOutPath,
            { value: info.amountNeedPay.add(1) }
          )
      ).revertedWith("VE2");
      ...
      await expect(
        mortgageContract
          ...
          .multiply(
            ...
            otherToFindOutPath,
            { value: info.amountNeedPay.sub(1) }
          )
      ).revertedWith("E");
      ...
      const inMatic = BigNumber.from("817068806908583810122");
      ...
      expect(user1Matic1.sub(user1Matic2).sub(gas)).eq(inMatic);
      ...
      expect(info.amountNeedPay).eq(inMatic);
      expect(info.tokenPay).eq(wmatic.address);
