...
describe("Mortgage.split.find", function () {
  ...
  describe("split", function () {
    ...
    it("xxx", async function () {
      ...
      const amountInMax = needFind.add(BigNumber.from(100));
      ...
      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, 0, amountInMax, findContract.address)
      ).revertedWith("E3");

      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, ospAmount5.mul(2), amountInMax, findContract.address)
      ).revertedWith("AE");

      await mortgageContract
        .connect(user1Wallet)
        .split(info.tokenId, ospAmount5, amountInMax, findContract.address);
      ...
      expect(user1Find3.sub(user1Find4)).eq(needFind.sub(2));
      ...
      expect(splitInfo.amountIn).eq(needFind.sub(2));
      expect(splitInfo.tokenIn).eq(findContract.address);
