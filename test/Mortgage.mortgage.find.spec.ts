...
describe("Mortgage.mortgage.find", function () {
  ...
  describe("mortgage", function () {
    ...
    it("find", async function () {
      ...
      const mortgageInfoFind = await mortgageContract
        .connect(user1Wallet)
      ...
      expect(user1Find2.sub(user1Find1)).eq(outFind);
      ...
      const position = await mortgageContract.positions(
        mortgageInfoFind.tokenId
      );
      expect(position.tokenId).eq(mortgageInfoFind.tokenId);
      expect(position.ospAsset).eq(osp1.address);
      expect(position.ospAmount).eq(ospAmount5);
      expect(await mortgageContract.ownerOf(mortgageInfoFind.tokenId)).eq(
        user1Wallet.address
      );

      expect(mortgageInfoFind.outFindAmount).eq(outFind);
      expect(mortgageInfoFind.amountOut).eq(outFind);
      expect(mortgageInfoFind.tokenOut).eq(findContract.address);
      ...
      const redeemFind = allFind;

      expect(user1Find2.sub(user1Find3)).eq(redeemFind);
      ...
      const position2 = await mortgageContract.positions(
        mortgageInfoFind.tokenId
      );
      ...
      await expect(
        mortgageContract.ownerOf(mortgageInfoFind.tokenId)
      ).revertedWith("ERC721: invalid token ID");

      expect(redeemInfo.inFindAmount).eq(allFind);
      expect(redeemInfo.amountIn).eq(allFind);
      expect(redeemInfo.tokenIn).eq(findContract.address);
