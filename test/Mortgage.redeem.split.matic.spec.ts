...
describe("Mortgage.redeem.split.matic", function () {
  ...
  describe("redeem", function () {
    ...
    it("xxx", async function () {
      ...
      const user2Balance = async function () {
        return (await ethers.provider.getBalance(user2Wallet.address)).sub(BigNumber.from(10).pow(18).mul(100))
      }
      ...
      const t1 = await mortgageContract
        ...
        .redeem(
          ...
          await user2Balance(),
          maticToFindOutPath,
          {gasPrice: gasPrice, value: await user2Balance()}
        );
      ...
      // console.log(1)
      // console.log(user2Matic1.sub(user2Matic2).sub(gas))
      // console.log(user2ReddemInfo.amountIn)
      // console.log(user2ReddemInfoSplit1.amountIn
      //   .add(user2ReddemInfoSplit2.amountIn)
      //   .add(user2ReddemInfoSplit3.amountIn))
      expect(user2Matic1.sub(user2Matic2).sub(gas))
        .eq(user2ReddemInfo.amountIn.add(521))
        .eq(
      ...
      console.log("matic", user2ReddemInfo.amountIn);

      expect(user2ReddemInfoSplit1.tokenIn).eq(wmatic.address);
      expect(user2ReddemInfoSplit2.tokenIn).eq(wmatic.address);
      expect(user2ReddemInfoSplit3.tokenIn).eq(wmatic.address);
