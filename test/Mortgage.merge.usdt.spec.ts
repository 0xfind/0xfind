...
describe("Mortgage.merge.usdt", function () {
  ...
  describe("merge", function () {
    ...
    it("xxx", async function () {
      ...
      const mortgageUsdt3 = await usdtContract.balanceOf(mortgageContract.address);
      ...
      const mergeInfo = await mortgageContract
        .connect(user1Wallet)
        .callStatic.merge(info1.tokenId, info2.tokenId, findToUsdtInPath);
      await mortgageContract
        .connect(user1Wallet)
        .merge(info1.tokenId, info2.tokenId, findToUsdtInPath);
      ...
      const mortgageUsdt4 = await usdtContract.balanceOf(mortgageContract.address);
      ...
      expect(user1Usdt4.sub(user1Usdt3)).eq(BigNumber.from("3275414022"));
      expect(earnUsdt4).eq(earnUsdt3);
      expect(mortgageUsdt4).eq(mortgageUsdt3);
