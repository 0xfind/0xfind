...
describe("Mortgage.cash.matic", function () {
  ...
  describe("cash", function () {
    ...
    const mortgageOsp3 = await osp1.balanceOf(mortgageContract.address);
      ...
      const findToMaticInPath =
        ...
        wmatic.address.slice(2);
      console.log(1);
      const cashInfoMatic = await mortgageContract
        .connect(user1Wallet)
        .callStatic.cash(mortgageStaticInfo.tokenId, ospAmount5, findToMaticInPath);
      const user1Wmatic1 = await wmatic.balanceOf(user1Wallet.address);
      const user1Matic1 = await user1Wallet.getBalance();
      console.log(2);
      await osp1
        .connect(deployWallet)
      ...
      expect(amountOut).eq(BigNumber.from("331417602685672325262697"));

      const t1 = await mortgageContract
        .connect(user1Wallet)
        .cash(mortgageStaticInfo.tokenId, ospAmount5, findToMaticInPath, {gasPrice: gasPrice});
      ...
      const redeemFind = BigNumber.from("106574803158593914169768");

      expect(user1Find4).eq(user1Find3);
      expect(earnFind4).eq(earnFind3);
      expect(mortgageFind4).eq(mortgageFind3);
      ...
      expect(user1Matic4.sub(user1Matic3).add(gas)).eq(
        cashInfoMatic.amountOut
      );
      ...
      const position2 = await mortgageContract.positions(mortgageStaticInfo.tokenId);
      expect(position2.ospAmount).eq(0);
      expect(position2.ospAsset).eq(ZERO_ADDRESS);

      expect(cashInfoMatic.outFindAmount).eq(amountOut.sub(redeemFind));
      expect(cashInfoMatic.tokenOut).eq(wmatic.address);
      expect(cashInfoMatic.amountOut).lt(cashInfoMatic.outFindAmount);

      expect(await findContract.totalSupply()).eq(findOldTotalSupply);

      expect(BigNumber.from("54106954098080434072184")).eq(redeemInfoMatic.amountIn)
      expect(BigNumber.from("111856090160388086332230")).eq(cashInfoMatic.amountOut)
