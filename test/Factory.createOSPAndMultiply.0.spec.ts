...
describe("Factory.createOSPAndMultiply.0", function () {
  ...
  describe("createOSPAndMultiply", function () {
    ...
    it("find", async function () {
      ...
      expect(staticInfo.amountNeedPay).eq(
        BigNumber.from("1609387179011089915998")
      );
      expect(user1Find1.sub(user1Find2)).eq(staticInfo.amountNeedPay);
      ...
      const fee = earnFind2.sub(earnFind1).sub(buyNFTFindAmount)
      expect(buyNFTFindAmount).eq(0)
      expect(earnFind2.sub(earnFind1)).eq(fee)

      expect(await findContract.totalSupply())
        .eq(
          findOldTotalSupply
            .add(AllFindAmount)
            .sub(staticInfo.amountNeedPay.sub(fee))
        )
        .eq(BigNumber.from("100000106574803158593914169768"));
