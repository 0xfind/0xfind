...
describe("Mortgage.split.weth", function () {
  ...
  describe("split", function () {
    ...
    before(async function () {
      ...
      await wethContract
        .connect(deployWallet)
        .transfer(user1Wallet.address, BigNumber.from(10).pow(18).mul(200));
    ...
    it("xxx", async function () {
      ...
      const amountInMax = BigNumber.from(10).pow(18).mul(200);
      ...
      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, 0, amountInMax, wethToFindOutPath)
      ).revertedWith("E3");

      await expect(
        mortgageContract
          .connect(user1Wallet)
          .split(info.tokenId, ospAmount5.mul(2), amountInMax, wethToFindOutPath)
      ).revertedWith("AE");

      await mortgageContract
        .connect(user1Wallet)
        .split(info.tokenId, ospAmount5, amountInMax, wethToFindOutPath);
      ...
      const mortgageWeth4 = await wethContract.balanceOf(
        mortgageContract.address
      );
      ...
      expect(user1Weth3.sub(user1Weth4)).eq(splitInfo.amountIn);
      ...
      expect(splitInfo.amountIn).eq(BigNumber.from("3245815514281667894"));
      expect(splitInfo.tokenIn).eq(wethContract.address);
