...
describe("Mortgage.redeem.split.approve", function () {
  ...
  describe("redeem", function () {
    ...
    it("xxx", async function () {
      ...
      expect(user2Find1.sub(user2Find2)).eq(user2ReddemInfoSplit1.inFindAmount);
      expect(user3Find2).eq(user3Find1);
      expect(mortgageFind2).eq(mortgageFind1).eq(0);
      expect(earnOsp2).eq(earnOsp1).eq(0);
      expect(user2Osp2.sub(user2Osp1)).eq(ospAmount1.mul(2));
      expect(user3Osp2).eq(user3Osp1);
      expect(mortgageOsp1.sub(mortgageOsp2)).eq(ospAmount1.mul(2));

      await mortgageContract.connect(user2Wallet).approve(user3Wallet.address, user2Osp1StaticInfo.tokenId);

      const user3ReddemInfoSplit2 = await mortgageContract
        .connect(user3Wallet)
        .callStatic.redeem(
          user2Osp1StaticInfo.tokenId,
          ospAmount1.mul(2),
          await findContract.balanceOf(user3Wallet.address),
          findContract.address
        );
      await mortgageContract
        .connect(user3Wallet)
      ...
      const user3ReddemInfoSplit3 = await mortgageContract
        .connect(user3Wallet)
        .callStatic.redeem(
          user2Osp1StaticInfo.tokenId,
          ospAmount1.mul(1),
      ...
      expect(user3Find2.sub(user3Find3)).eq(
        user3ReddemInfoSplit2.inFindAmount.add(
          user3ReddemInfoSplit3.inFindAmount
        )
      );
      expect(mortgageFind3).eq(mortgageFind2).eq(0);
      expect(earnOsp3).eq(earnOsp2).eq(0);
      expect(user2Osp3).eq(user2Osp2);
      expect(user3Osp3.sub(user3Osp2)).eq(
        ospAmount1.mul(3)
      );
      expect(mortgageOsp2.sub(mortgageOsp3)).eq(
        ospAmount1.mul(3)
      );
      ...
      expect(user2ReddemInfo.amountIn).eq(
        user2ReddemInfoSplit1.amountIn
          .add(user3ReddemInfoSplit2.amountIn)
          .add(user3ReddemInfoSplit3.amountIn)
      );
      expect(user2ReddemInfoSplit1.amountIn).gt(user3ReddemInfoSplit2.amountIn);
