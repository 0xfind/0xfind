...
describe("OspPool.ospFind.1.init", function () {
  let wallets;
  ...
  it("projectId2OspToken token2OspInfo ospLpTokenIdList", async function () {
    ...
    expect(ospLpTokenIdList.length).eq(iospLpTokenIdList.length).eq(1);
  ...
  it("osp pool info", async function () {
    ...
    expect(await ospPool.fee())
      .eq(DEFAULT_OSP_POOL_CONFIG_1.fee)
      .eq(10000);
    ...
    expect(tmp1).lt(1000);
    expect(tmp2.add(tmp1)).eq(BigNumber.from(10).pow(18).mul(100_0000));
  });
  ...
  it("osp nft info", async function () {
    ...
    expect(cinfo.token)
      ...
      .eq(icinfo[4])
      .eq(osp1.address);
    expect(cinfo.percent).eq(cinfo[5]).eq(icinfo.percent).eq(icinfo[5]).eq(1500);
    ...
    expect(oinfo.token)
      ...
      .eq(ioinfo[4])
      .eq(osp1.address);
    expect(oinfo.percent).eq(oinfo[5]).eq(ioinfo.percent).eq(ioinfo[5]).eq(8500);
  ...
  it("osp lp token info", async function () {
    ...
    for (let index = 0; index < ospLpTokenIdList.length; index++) {
      ...
      expect(position.fee).eq(DEFAULT_OSP_POOL_CONFIG_1.fee).eq(config.fee).eq(10000);
    }
  });
