...
describe("Mortgage.redeem.split.usdt", function () {
  ...
  describe("redeem", function () {
    ...
    it("xxx", async function () {
      ...
      await usdtContract
        .connect(deployWallet)
        .transfer(user2Wallet.address, BigNumber.from(10).pow(6).mul(20_0000));
      ...
      await usdtContract
        .connect(user2Wallet)
        .approve(mortgageContract.address, await usdtContract.totalSupply());
      const user2ReddemInfo = await mortgageContract
        .connect(user2Wallet)
        .callStatic.redeem(
          user2Osp1StaticInfo.tokenId,
          ospAmount5,
          await usdtContract.balanceOf(user2Wallet.address),
          otherToFindOutPath
        );
      ...
      console.log("usdt", user2ReddemInfo.amountIn);

      expect(user2ReddemInfoSplit1.tokenIn).eq(usdtContract.address);
      expect(user2ReddemInfoSplit2.tokenIn).eq(usdtContract.address);
      expect(user2ReddemInfoSplit3.tokenIn).eq(usdtContract.address);
