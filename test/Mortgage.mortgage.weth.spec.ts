...
describe("Mortgage.mortgage.weth", function () {
  ...
  describe("mortgage", function () {
    ...
    it("weth", async function () {
      ...
      expect(mortgageInfo.amountOut).eq(outWeth);
      ...
      const redeemWeth = BigNumber.from("106584466326875649191");

      expect(user1Find2).eq(user1Find3);
      expect(earnFind3).eq(earnFind2);
      expect(mortgageFind3).eq(mortgageFind2).eq(0);

      expect(user1Weth2.sub(user1Weth3)).eq(redeemWeth);
      expect(earnWeth3).eq(earnWeth2).eq(0);
      expect(mortgageWeth3).eq(mortgageWeth3).eq(0);
      ...
      expect(redeemInfo.amountIn).eq(redeemWeth);
