...
describe("Mortgage.multiply.usdt", function () {
  ...
  describe("multiply", function () {
    ...
    it("xxx", async function () {
      ...
      expect(outFind.add(feeFind)).eq(redeemFind);
      ...
      const inFind = BigNumber.from("1609387179011089915998");
      const inUsdt = BigNumber.from("1665849258");

      expect(user1Find1).eq(user1Find2);
      expect(earnFind2.sub(earnFind1)).eq(feeFind);
      expect(mortgageFind2).eq(mortgageFind1).eq(0);

      expect(user1Usdt1.sub(user1Usdt2)).eq(inUsdt);
      ...
      expect(info.positionOspAmountDelta).eq(ospAmount5);
      expect(info.payFindAmount).eq(inFind);
      expect(info.amountNeedPay).eq(inUsdt);
      expect(info.tokenPay).eq(usdtContract.address);
      ...
      expect(positions[0].ospAmount).eq(ospAmount5);
