...
describe("Mortgage.merge.matic", function () {
  ...
  describe("merge", function () {
    ...
    before(async function () {
      ...
      await osp1
        .connect(deployWallet)
        .transfer(user1Wallet.address, ospAmount5.mul(2));
    });
    ...
    it("xxx", async function () {
      ...
      await osp1
        .connect(user1Wallet)
        .approve(mortgageContract.address, ospAmount5.mul(2));
      ...
      expect(user1Find2.sub(user1Find1)).eq(
        BigNumber.from("106041929142800944598920").mul(2)
      );

      expect(await findContract.totalSupply()).eq(
        findOldTotalSupply.add(
          BigNumber.from("106574803158593914169768").mul(2)
        )
      );

      const position1 = await mortgageContract.positions(info1.tokenId);
      expect(position1.ospAmount).eq(ospAmount5);
      const position2 = await mortgageContract.positions(info1.tokenId);
      expect(position2.ospAmount).eq(ospAmount5);
      ...
      const mortgageWmatic3 = await wmatic.balanceOf(mortgageContract.address);
      ...
      const mortgageMatic3 = await ethers.provider.getBalance(mortgageContract.address);
      ...
      const mergeInfo = await mortgageContract
        .connect(user1Wallet)
        .callStatic.merge(info1.tokenId, info2.tokenId, findToMaticInPath);
      const t1 = await mortgageContract
        .connect(user1Wallet)
        .merge(info1.tokenId, info2.tokenId, findToMaticInPath, {gasPrice: gasPrice});
      ...
      const mortgageWmatic4 = await wmatic.balanceOf(mortgageContract.address);
      ...
      const mortgageMatic4 = await ethers.provider.getBalance(mortgageContract.address);
      ...
      expect(earnFind4.sub(earnFind3)).eq(
        BigNumber.from("16227606208769253321")
      );
      expect(mortgageFind4).eq(mortgageFind3).eq(0);

      expect(user1Wmatic4).eq(user1Wmatic3).eq(0);
      expect(earnWmatic4).eq(earnWmatic3).eq(0);
      expect(mortgageWmatic4).eq(mortgageWmatic3).eq(0);

      expect(user1Matic4.sub(user1Matic3).add(gas)).eq(BigNumber.from("1606531093240892233941"));
      expect(earnMatic4).eq(earnMatic3);
      expect(mortgageMatic4).eq(mortgageMatic3);

      expect(user1Osp4).eq(user1Osp3).eq(0);
      expect(earnOsp4).eq(earnOsp3).eq(0);
      expect(mortgageOsp4).eq(mortgageOsp3).eq(ospAmount5.mul(2));

      const position1_2 = await mortgageContract.positions(info1.tokenId);
      expect(position1_2.ospAmount).eq(ospAmount5.mul(2));
      expect(position1_2.ospAsset).eq(osp1.address);
      expect(position1_2.tokenId).eq(info1.tokenId);

      const position2_2 = await mortgageContract.positions(info2.tokenId);
      expect(position2_2.ospAmount).eq(0);
      expect(position2_2.ospAsset).eq(ZERO_ADDRESS);
      expect(position2_2.tokenId).eq(0);

      expect(mergeInfo.tokenOut).eq(
        wmatic.address
      );
      expect(mergeInfo.amountOut).eq(
        BigNumber.from("1606531093240892233941")
      );
      expect(mergeInfo.outFind).eq(
        BigNumber.from("3229293635545081411054")
      );


      expect(await findContract.totalSupply()).eq(
        findOldTotalSupply
          .add(BigNumber.from("106574803158593914169768"))
          .add(BigNumber.from("109820324400347764834143"))
      );
