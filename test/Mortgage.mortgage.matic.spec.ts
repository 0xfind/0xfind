...
describe("Mortgage.mortgage.matic", function () {
  ...
  describe("mortgage", function () {
    ...
    it("matic", async function () {
      ...
      const user1Matic1 = await ethers.provider.getBalance(user1Wallet.address);
      ...
      const user1Wmatic2 = await wmatic.balanceOf(user1Wallet.address);
      ...
      const user1Matic2 = await ethers.provider.getBalance(user1Wallet.address);
      ...
      const outMatic = BigNumber.from("52754411045131079432184")
      ...
      expect(user1Wmatic2).eq(user1Wmatic1).eq(0);
      expect(earnWmatic2).eq(earnWmatic1).eq(0);
      expect(mortgageWmatic2).eq(mortgageWmatic1).eq(0);

      expect(user1Matic2.sub(user1Matic1).add(gas)).eq(outMatic);
      ...
      expect(await mortgageContract.ownerOf(mortgageInfo.tokenId)).eq(user1Wallet.address);

      expect(mortgageInfo.outFindAmount).eq(outFind);
      expect(mortgageInfo.amountOut).eq(outMatic);
      expect(mortgageInfo.tokenOut).eq(wmatic.address);
      ...
      const redeemMatic = BigNumber.from("54106841069733337102558");
      ...
      expect(user1Wmatic3).eq(user1Wmatic2).eq(0);
      expect(earnWmatic3).eq(earnWmatic2).eq(0);
      expect(mortgageWmatic3).eq(mortgageWmatic2).eq(0);

      expect(user1Matic2.sub(user1Matic3).sub(gas2)).eq(redeemMatic);
      expect(earnMatic3).eq(earnMatic3).eq(0);
      expect(mortgageMatic3).eq(mortgageMatic3).eq(0);
      ...
      expect(redeemInfo.amountIn).eq(redeemMatic);
      expect(redeemInfo.tokenIn).eq(wmatic.address);
