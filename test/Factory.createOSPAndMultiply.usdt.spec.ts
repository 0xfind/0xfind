...
describe("Factory.createOSPAndMultiply.usdt", function () {
  ...
  describe("createOSPAndMultiply", function () {
    ...
    it("usdt", async function () {
      ...
      const buyNFTTokenAmountMax = BigNumber.from(10).pow(6).mul(10);
      ...
      const amountPayMax = BigNumber.from("1665849258");

      await usdtContract
        .connect(user1Wallet)
        .approve(
          factoryContract.address,
          amountPayMax.add(osp1Params.buyNFTTokenAmountMax)
        );
      ...
      const factoryUsdt1 = await usdtContract.balanceOf(
        factoryContract.address
      );
      ...
      const factoryUsdt2 = await usdtContract.balanceOf(
        factoryContract.address
      );
      ...
      expect(staticInfo.nftNeedPay).eq("3105250");

      expect(staticInfo.positionOspAmountDelta).eq(ospAmount5);
      expect(staticInfo.tokenPay).eq(usdtContract.address);

      expect(staticInfo.amountNeedPay).eq(
        BigNumber.from("1665849258")
      );

      expect(user1Find1).eq(user1Find2);
      expect(user1Usdt1.sub(user1Usdt2)).eq(
      ...
      expect(await findContract.balanceOf(factoryContract.address)).eq(928);
      expect(await usdtContract.balanceOf(factoryContract.address)).eq(0);
