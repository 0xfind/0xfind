...
describe("Mortgage.mortgage.add.approve", function () {
  ...
  describe("mortgage", function () {
    ...
    it("xxx", async function () {
      // user1 user2 user3 get osp get find
      ...
      await findContract
        ...
        await findContract
        .connect(deployWallet)
        .transfer(user3Wallet.address, BigNumber.from(10).pow(18).mul(20_0000));
        await osp1
        .connect(deployWallet)
      ...
      // user3 mortgage 2
      // user3 mortgage 1
      const earn2Find1 = await findContract.balanceOf(earnContract.address);
      const user2Find1 = await findContract.balanceOf(user2Wallet.address);
      const user3Find1 = await findContract.balanceOf(user3Wallet.address);
      
      const earn2Osp1 = await osp1.balanceOf(earnContract.address);
      const user2Osp1 = await osp1.balanceOf(user2Wallet.address);
      const user3Osp1 = await osp1.balanceOf(user3Wallet.address);
      ...
      const earn2Find2 = await findContract.balanceOf(earnContract.address);
      const user2Find2 = await findContract.balanceOf(user2Wallet.address);
      const user3Find2 = await findContract.balanceOf(user3Wallet.address);
      
      const earn2Osp2 = await osp1.balanceOf(earnContract.address);
      const user2Osp2 = await osp1.balanceOf(user2Wallet.address);
      const user3Osp2 = await osp1.balanceOf(user3Wallet.address);

      expect(earn2Find2).gt(earn2Find1);
      expect(user2Find2).gt(user2Find1);
      expect(user3Find2).eq(user3Find1);
      
      expect(earn2Osp2).eq(earn2Osp1);
      expect(user2Osp1.sub(user2Osp2)).eq(ospAmount1.mul(2))
      expect(user3Osp2).eq(user3Osp1);

      await mortgageContract.connect(user2Wallet).approve(user3Wallet.address, user2MortgageInfo.tokenId);

      const user3MortgageInfoSplit2 = await mortgageContract
        .connect(user3Wallet)
      ...
      const user3MortgageInfoSplit3 = await mortgageContract
        .connect(user3Wallet)
        .callStatic.mortgageAdd(
          user2MortgageInfo.tokenId,
          ospAmount1.mul(1),
      ...
      const earn2Find3 = await findContract.balanceOf(earnContract.address);
      const user2Find3 = await findContract.balanceOf(user2Wallet.address);
      const user3Find3 = await findContract.balanceOf(user3Wallet.address);
      
      const earn2Osp3 = await osp1.balanceOf(earnContract.address);
      const user2Osp3 = await osp1.balanceOf(user2Wallet.address);
      const user3Osp3 = await osp1.balanceOf(user3Wallet.address);
      
      expect(user3Find3).gt(user3Find2);
      expect(user2Find3).eq(user2Find2);
      
      expect(earn2Osp3).eq(earn2Osp1);
      expect(user2Osp1.sub(user2Osp3)).eq(ospAmount1.mul(2))
      expect(user3Osp1.sub(user3Osp3)).eq(ospAmount1.mul(3))
 
      const mintTotalSupply = await findContract.totalSupply();

      // sub1wei is the expected deviation within the mortgage pool
      expect(user1MintTotalSupply).eq(mintTotalSupply.sub(1));
      ...
      expect(earn2Find3.sub(earn2Find1)).eq(earn1Find2.sub(earn1Find1).sub(1));

      // sub1wei is the expected deviation within the mortgage pool
      expect(user2Find2.sub(user2Find1).add(user3Find3.sub(user3Find2)).sub(1))
        .eq(user2MortgageInfo.amountOut.add(1))
        .eq(
          user2MortgageInfoSplit1.amountOut
            .add(user3MortgageInfoSplit2.amountOut)
            .add(user3MortgageInfoSplit3.amountOut)
            .sub(1)
        );
      expect(user3MortgageInfoSplit2.amountOut).gt(
        user2MortgageInfoSplit1.amountOut
      );
