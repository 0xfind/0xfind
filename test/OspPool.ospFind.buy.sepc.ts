...
describe("OspPool.ospFind.buy", function () {
  let wallets;
  ...
  const getOspParams = function(index: any) {
    return {
        base: {
  ...
  before(async function () {
    ...
    while(true) {
        ...
        if(osp1.address < findContract.address){
            break
        }
    }
  ...
  it("buy", async function () {
    ...
    console.log("0 is osp, 1 is find");
    expect(await pool.fee()).eq(config.fee).eq(10000);
    expect(await pool.token0()).eq(osp1.address);
    expect(await pool.token1()).eq(findContract.address);
    ...
    const osp1Amount1p = osp1TotalSupply.div(100);
    await findContract.connect(deployWallet).approve(swapRouter.address, await findContract.totalSupply());
    // buy 1 osp
    ...
    // buy 10%
    const buy10pInfo = await buyOsp(osp1Amount1p.mul(10))
    console.log("============= buy 10% =============")
    const buy1info2 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info2.find.mul(99).div(100).toString())

    // buy 10%
    const buy20pInfo = await buyOsp(osp1Amount1p.mul(10))
    console.log("============= buy 20% =============")
    const buy1info3 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info3.find.mul(99).div(100).toString())

    // buy 10%
    const buy30pInfo = await buyOsp(osp1Amount1p.mul(10))
    console.log("============= buy 30% =============")
    const buy1info4 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info4.find.mul(99).div(100).toString())

    // buy 10%
    const buy40pInfo = await buyOsp(osp1Amount1p.mul(10))
    console.log("============= buy 40% =============")
    const buy1info5 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info5.find.mul(99).div(100).toString())

    // buy 20%
    const buy60pInfo = await buyOsp(osp1Amount1p.mul(20))
    console.log("============= buy 60% =============")
    const buy1info6 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info6.find.mul(99).div(100).toString())

    // buy 20%
    const buy80pInfo = await buyOsp(osp1Amount1p.mul(20))
    console.log("============= buy 80% =============")
    const buy1info7 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info7.find.mul(99).div(100).toString())

    // buy 20%
    const buy100pInfo = await buyOsp(osp1Amount1p.mul(20).sub(100).sub(BigNumber.from(10).pow(18).mul(1)))
    console.log("============= buy 100% =============")
    const buy1info8 = await buyOspCallStatic(BigNumber.from(10).pow(18).mul(1));
    console.log(buy1info8.find.mul(99).div(100).toString())
  });
});
