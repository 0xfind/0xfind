...
describe("Factory.createOSP.0", function () {
  ...
  describe("createOSP", function () {
    ...
    it("find", async function () {
      ...
      const buyNFTFindAmount = 0;
      const buyNFTTokenAmountMax = 0;
      ...
      const staticInfo = await factoryContract
        .connect(user1Wallet)
        .callStatic.createOSP(osp1Params);

      await factoryContract.connect(user1Wallet).createOSP(osp1Params);
      ...
      expect(staticInfo.tokenPay).eq(findContract.address);

      expect(user1Find1.sub(user1Find2)).eq(staticInfo.nftNeedPay).eq(0);
      ...
      const osp2Params = {
        base: {
          name: "twitter.com/test",
          symbol: "@test",
          projectId: "twitter/123",
          stars: 1,
          poolConfigIndex: 0,
          nftPercentConfigIndex: 0,
        },
        ...
        buyNFTTokenAmountMax: buyNFTTokenAmountMax,
        buyNFTFindAmount: buyNFTFindAmount,
        tokenToFindOutPath: wmatic.address,
        signature: "",
      };
      ...
      const user1wmatic3 = await wmatic.balanceOf(user1Wallet.address);
      const earnwmatic3 = await wmatic.balanceOf(earnContract.address);
      const user1Matic3 = await ethers.provider.getBalance(user1Wallet.address); 
      const earnMatic3 = await ethers.provider.getBalance(earnContract.address); 

      const factoryFind3 = await findContract.balanceOf(
        factoryContract.address
      );

      const staticInfo2 = await factoryContract
        .connect(user1Wallet)
        .callStatic.createOSP(osp2Params);

      let gasPrice = BigNumber.from("900000000");
      const t1 = await factoryContract.connect(user1Wallet).createOSP(osp2Params, {gasPrice: gasPrice});
      const t1w = await t1.wait();
      
      let gas = t1w.gasUsed.mul(gasPrice);
      ...
      const user1wmatic4 = await wmatic.balanceOf(user1Wallet.address);
      const earnwmatic4 = await wmatic.balanceOf(earnContract.address);
      const user1Matic4 = await ethers.provider.getBalance(user1Wallet.address); 
      const earnMatic4 = await ethers.provider.getBalance(earnContract.address); 

      const factoryFind4 = await findContract.balanceOf(
        factoryContract.address
      );

      expect(staticInfo2.tokenPay).eq(wmatic.address);

      expect(user1Find3.sub(user1Find4)).eq(0);
      expect(user1wmatic3.sub(user1wmatic4)).eq(0);
      expect(earnFind3.sub(earnFind4)).eq(0);
      expect(earnwmatic4.sub(earnwmatic3)).eq(0);
      expect(earnMatic4).eq(earnMatic3);
      expect(factoryFind4).eq(factoryFind3);

      expect(user1Matic3.sub(user1Matic4).sub(gas)).eq(staticInfo2.nftNeedPay).eq(0);

      const osp2Info1 = await factoryContract.token2OspInfo(osp2.address);
      const osp2Info2 = await findnftContract.tokenId2Info(osp2Info1.cnftTokenId);
      console.log("osp2Info2.name", osp2Info2.name)
      console.log("osp2Info2.symbol", osp2Info2.symbol)
      console.log("osp2Info2.projectId", osp2Info2.projectId)
      console.log("osp2.name", await osp2.name())
      console.log("osp2.symbol", await osp2.symbol())
      expect(osp2Info2.name).eq(osp2Params.base.name).eq(await osp2.name())
      expect(osp2Info2.symbol).eq(osp2Params.base.symbol).eq(await osp2.symbol())
      expect(osp2Info2.projectId).eq(osp2Params.base.projectId)
          ...
          ethers.utils.keccak256(
            ethers.utils.defaultAbiCoder.encode(
              ...
              [
                osp3Params.base,
                osp3Params.deadline,
                osp3Params.buyNFTTokenAmountMax,
                osp3Params.buyNFTFindAmount,
                osp3Params.tokenToFindOutPath,
                user1Wallet.address,
              ]
            )
          )
      ...
      await expect(
        factoryContract.connect(user1Wallet).createOSP(osp3Params, {gasPrice: gasPrice})
      ).revertedWith("E1");
          ...
          ethers.utils.keccak256(
            ethers.utils.defaultAbiCoder.encode(
              ...
              [
                osp4Params.base,
                osp4Params.deadline,
                osp4Params.buyNFTTokenAmountMax,
                osp4Params.buyNFTFindAmount,
                osp4Params.tokenToFindOutPath,
                user1Wallet.address,
              ]
            )
          )
      ...
      await expect(
        factoryContract.connect(user1Wallet).createOSP(osp4Params, {gasPrice: gasPrice, value: 2})
      ).revertedWith("E2");
